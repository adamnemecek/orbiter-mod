...
void draw_it(INT *sol)
{
	...
	cout << "\\begin{tikzpicture}[x=1cm, y=1cm, semitransparent, scale=0.5]" << endl;
	cout << "\\draw[step=1cm, line width=0.3mm, black!30!white] (0,0) grid (5cm,5cm);" << endl;
	for (u = 0; u < sol_length; u++) {
		j = sol[u];
		decode_piece(j, h, r, t);
		//cout << "% j=" << j << " h=" << h << " r=" << r << " t=" << t << endl;
