...
void draw_begining_callback(layered_graph *LG, mp_graphics *G, INT x_max, INT y_max, INT dx, INT dy)
{
	...
	for (l = 0; l < LG->nb_layers + boxed_group_size - 1; l += boxed_group_size) {
		...
		LG->coordinates(LG->L[l1].Nodes[0].id, x_max, y_max, x, y);
		y0 = y;
		Px[0] = x;
		Py[0] = y;
		LG->coordinates(LG->L[l2].Nodes[0].id, x_max, y_max, x, y);
		Px[1] = x;
		Py[1] = y;
		y1 = y;
		...
		for (ll = l1; ll <= l2; ll++) {
			for (j = 0; j < LG->L[ll].nb_nodes; j++) {
				LG->coordinates(LG->L[ll].Nodes[j].id, x_max, y_max, x, y);
				x0 = MINIMUM(x0, x);
				x1 = MAXIMUM(x1, x);
				}
			}
		...
		for (i = 0; i < LG->nb_layers; i++) {
			for (j = 0; j < LG->L[i].nb_nodes; j++) {
				if (LG->L[i].Nodes[j].label) {
					...
					LG->coordinates(LG->L[i].Nodes[j].id, x_max, y_max, x, y);
