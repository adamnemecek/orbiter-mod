...
int main(int argc, const char **argv)
{
	...
	if (Gen.f_draw_full_poset) {
		Gen.gen->draw_poset_full(Gen.gen->fname_base, depth, Gen.n /* data1 */, f_embedded, Gen.gen->verbose_level);
		}
	...
	if (Gen.f_draw_graphs) {
		...
		for (level = 0; level <= Gen.gen->depth; level++) {
			Gen.draw_graphs(level, Gen.scale, xmax_in, ymax_in, xmax, ymax, Gen.f_embedded, Gen.gen->verbose_level);
			}
		}
	...
	if (Gen.f_draw_graphs_at_level) {
		...
		Gen.draw_graphs(Gen.level, Gen.scale, xmax_in, ymax_in, xmax, ymax, Gen.f_embedded, Gen.gen->verbose_level);
		}

	if (Gen.f_draw_level_graph) {
		Gen.gen->draw_level_graph(Gen.gen->fname_base, Gen.gen->depth, Gen.n /* data1 */, Gen.level_graph_level, f_embedded, Gen.gen->verbose_level - 3);
		}
