...
INT generator::classify(INT from_level, INT to_level, 
...
// returns TRUE if there is at least one orbit at level to_level, FALSE otherwise
{
	...
	if (f_v) {
		cout << "generator::classify from " << from_level << " to " << to_level << endl;
	...
	if (f_v) {
		cout << "generator::classify from " << from_level << " to " << to_level << " done" << endl;
		}
	return to_level;
}
