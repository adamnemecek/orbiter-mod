...
// calls init_extension_node
{
	...
	ret = init_extension_node(verbose_level - 1);
...
// returns FALSE if the set is not canonical (provided f_indicate_not_canonicals is TRUE)
{
	...
	if (G) {
		cout << "upstep_work::init_extension_node G" << endl;
		exit(1);
		}
	if (H) {
		cout << "upstep_work::init_extension_node H" << endl;
		exit(1);
		}
	G = new group;
	H = new group;
	...
	if (f_vvv) {
		if (gen->f_print_function) {
			(*gen->print_function)(cout, size, gen->S, gen->print_function_data);
			}
		}
	if (f_vv) {
	...
	cout << "b4 preping H: pt=" << pt << ",orblen" << pt_orbit_len<< ",go_G"<<go_G<<",go_H"<<go_H<<endl;
	...
	if (gen->f_on_subspaces) {
#if 0
		if (cur == 26) {
			cout << "upstep_work::init_extension_node Node=26 before upstep_subspace_action" << endl;
			}
#endif
		if (f_v) {
			gen->print_level_extension_info(size, prev, prev_ex);
...
#if 0
		if (cur == 26) {
			cout << "upstep_work::init_extension_node Node=26 after upstep_subspace_action" << endl;
			}
#endif
		}
	else {
		if (f_v) {
			gen->print_level_extension_info(size, prev, prev_ex);
			INT_vec_print(cout, gen->S, size);
			cout << "upstep_work::init_extension_node calling upstep_for_sets" << endl;
			}
...
// returns FALSE if the set is not canonical (provided f_indicate_not_canonicals is TRUE)
{
	...
	if (coset_table) {
		cout << "upstep_work::upstep_for_sets coset_table" << endl;
		exit(1);
		}
	nb_cosets = size;
	...
	for (coset = 0; coset < size - 1; coset++) {
		if (f_v) {
			cout << "upstep_work::upstep_for_sets coset=" << coset << endl;
			}
...
void print_coset_table(coset_table_entry *coset_table, INT len)
{
	...
	for (i = 0; i < len; i++) {
		cout << setw(3) << i << " : "
			...
			<< setw(5) << coset_table[i].nb_times_mult_called << "/"
			<< setw(5) << coset_table[i].nb_times_invert_called << "/"
			<< setw(5) << coset_table[i].nb_times_retrieve_called << " : "
