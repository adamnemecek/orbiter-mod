...
void recoordinatize::do_recoordinatize(INT i1, INT i2, INT i3, INT verbose_level)
{
	...
	Grass->unrank_INT_here(M, i1, verbose_level - 4);
	Grass->unrank_INT_here(M + k * n, i2, verbose_level - 4);
	Grass->unrank_INT_here(M + 2 * k * n, i3, verbose_level - 4);
	if (f_vv) {
		cout << "M:" << endl;
		print_integer_matrix_width(cout, M, 3 * k, n, n, F->log10_of_q + 1);
		}
	INT_vec_copy(M, AA, n * n);
	F->matrix_inverse(AA, AAv, n, verbose_level - 1);
	if (f_vv) {
		cout << "AAv:" << endl;
	...
	F->matrix_inverse(TT, TTv, k, verbose_level - 1);
	if (f_vv) {
		cout << "TTv:" << endl;
	...
	j1 = Grass->rank_INT_here(M1, verbose_level - 4);
	j2 = Grass->rank_INT_here(M1 + k * n, verbose_level - 4);
	j3 = Grass->rank_INT_here(M1 + 2 * k * n, verbose_level - 4);
	if (f_v) {
...
void recoordinatize::stabilizer_of_first_three(strong_generators *&Strong_gens /*vector_ge *&gens, INT *&tl */, INT verbose_level)
{
	INT f_v = (verbose_level >= 1);
	...
	generators_to_strong_generators(A, 
		...
		gens2, Strong_gens /* gens, tl */, verbose_level - 1);
		// in ACTION/action_global.C
